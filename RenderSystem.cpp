...
RenderSystem::RenderSystem()
{
	...
	UINT no_driver_types = ARRAYSIZE(driver_types);
	UINT no_feature_levels = ARRAYSIZE(feature_levels);

	HRESULT res = 0;
	for (UINT driver_type_index = 0; driver_type_index < no_driver_types;) {
		res = D3D11CreateDevice(NULL, driver_types[driver_type_index], NULL, NULL, feature_levels, no_feature_levels,
	...
	if (FAILED(res)) {
		throw std::exception("Render system failed to be created!");
	}
...
bool RenderSystem::compileVertexShader(const wchar_t* file_name, const char* entry_point_name, void** shader_byte_code, size_t* byte_code_size)
{
	...
	if (!SUCCEEDED(D3DCompileFromFile(file_name, nullptr, nullptr, entry_point_name, "vs_5_0",
		0, 0, &m_blob, &error_blob)))
	{
...
{
	ID3DBlob* error_blob = nullptr;
	if (!SUCCEEDED(D3DCompileFromFile(file_name, nullptr, nullptr, entry_point_name, "ps_5_0",
		0, 0, &m_blob, &error_blob)))
	{
		if (error_blob) error_blob->Release();
		return false;
...
{
	if (cull_front) {
		m_imm_context->RSSetState(m_rasterizer_state_cull_front);
	}
	else {
		m_imm_context->RSSetState(m_rasterizer_state_cull_back);
	}
}

void RenderSystem::initRasterizer()
{
	...
	m_d3d_device->CreateRasterizerState(&desc, &m_rasterizer_state_cull_front);
	//back face culling rasterizer
	desc.CullMode = D3D11_CULL_BACK;
	m_d3d_device->CreateRasterizerState(&desc, &m_rasterizer_state_cull_back);
}
